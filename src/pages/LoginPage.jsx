...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--background)' }} className="min-h-screen fade-in">
      {/* Panel izquierdo */}
      <div style={{ flex: 1, background: 'var(--primary)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 64px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -80, right: -80, width: 320, height: 320, borderRadius: '50%', background: 'rgba(4,120,87,.12)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -60, left: -60, width: 220, height: 220, borderRadius: '50%', background: 'rgba(4,120,87,.08)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 400 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 48 }}>
            ...
            <span style={{ fontSize: 22, fontWeight: 800, color: 'var(--primary-foreground)' }}>VetCore</span>
          </div>
          <h1 style={{ fontSize: 30, fontWeight: 800, color: 'var(--primary-foreground)', lineHeight: 1.2, marginBottom: 14 }}>
            Gestión veterinaria<br /><span style={{ color: 'var(--accent)' }}>centralizada.</span>
          </h1>
          <p style={{ color: 'var(--muted)', fontSize: 14, lineHeight: 1.6 }}>Mascotas, citas, consultas, medicamentos y facturación en un solo sistema.</p>
          <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[['🐾', 'Registro completo de pacientes y propietarios'], ['📅', 'Agenda y control de citas médicas'], ['💊', 'Inventario de medicamentos en tiempo real']].map(([icon, text]) => (
              <div key={icon} style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--muted)', fontSize: 13 }}>
      ...
      <div style={{ width: 440, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        <div style={{ width: '100%', maxWidth: 360, animation: 'fadeIn .3s ease' }}>
          ...
          {/* Demo */}
          <div style={{ marginTop: 28, padding: 16, background: 'var(--surface-inset)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', color: 'var(--muted)' }}>
