...
export function VeterinariosPage() {
  const { can } = useAuth()
  const { data, loading, refetch } = useFetch('/veterinarios')
      ...
      {showModal && (
        <Modal title={editing ? 'Editar Veterinario' : 'Nuevo Veterinario'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <FormRow>
              <FormCol><Input label="Nombres *" value={form.nombres} error={errors.nombres} allowPattern={PATTERNS.soloLetras} onChange={e => set('nombres', e.target.value)} /></FormCol>
              <FormCol><Input label="Apellidos *" value={form.apellidos} error={errors.apellidos} allowPattern={PATTERNS.soloLetras} onChange={e => set('apellidos', e.target.value)} /></FormCol>
            </FormRow>
            <FormRow>
              ...
              <FormCol><Input label="Email *" type="email" value={form.email} error={errors.email} onChange={e => set('email', e.target.value)} placeholder="dr@vetcore.com" /></FormCol>
            </FormRow>
...
export function EspecialidadesPage() {
  ...
  const handleDelete = async (e) => {
    if (!confirm(`¿Eliminar la especialidad "${e.nombre}"? Asegúrate de que no tenga veterinarios asignados.`)) return
    ...
    <div className="fade-in">
      ...
      {showModal && (
        <Modal title={editing ? 'Editar Especialidad' : 'Nueva Especialidad'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Input label="Nombre *" value={form.nombre} error={errors.nombre} allowPattern={PATTERNS.soloLetras} onChange={e => set('nombre', e.target.value)} placeholder="Ej: Cardiología" />
            <Input label="Descripción" value={form.descripcion} onChange={e => set('descripcion', e.target.value)} placeholder="Ej: Enfermedades del corazón" maxLength={255} />
            {apiError && <Alert message={apiError} />}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
              <Btn variant="ghost" onClick={() => setShowModal(false)}>Cancelar</Btn>
              <Btn onClick={handleSave} disabled={saving}>{saving ? 'Guardando…' : editing ? 'Actualizar' : 'Guardar'}</Btn>
    ...
    <div className="fade-in">
      ...
      {showModal && (
        <Modal title={editing ? 'Editar Especie' : 'Nueva Especie'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Input label="Nombre *" value={form.nombre} error={errors.nombre} allowPattern={PATTERNS.soloLetras} onChange={e => set('nombre', e.target.value)} placeholder="Ej: Conejo" />
            <Input label="Descripción" value={form.descripcion} onChange={e => set('descripcion', e.target.value)} placeholder="Nombre científico o descripción" maxLength={255} />
            {apiError && <Alert message={apiError} />}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
              <Btn variant="ghost" onClick={() => setShowModal(false)}>Cancelar</Btn>
              <Btn onClick={handleSave} disabled={saving}>{saving ? 'Guardando…' : editing ? 'Actualizar' : 'Guardar'}</Btn>
...
const MED_SCHEMA = {
  ...
  precio_unit: [rules.required, (v) => rules.numero(v, 0)(v)],
  stock:       [(v) => v !== '' ? rules.numero(v, 0)(v) : ''],
}

export function MedicamentosPage() {
  const { can } = useAuth()
  const { data, loading, refetch } = useFetch('/medicamentos')
  const [showModal, setShowModal] = useState(false)
  const [editing, setEditing] = useState(null)
  const [saving, setSaving] = useState(false)
  const [apiError, setApiError] = useState('')
    ...
    <div className="fade-in">
      ...
      {showModal && (
        <Modal title={editing ? 'Editar Medicamento' : 'Nuevo Medicamento'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Input label="Nombre *" value={form.nombre} error={errors.nombre} onChange={e => set('nombre', e.target.value)} placeholder="Ej: Amoxicilina 500mg" />
            <FormRow>
              <FormCol><Input label="Principio activo" value={form.principio_act} onChange={e => set('principio_act', e.target.value)} placeholder="Ej: Amoxicilina" /></FormCol>
              <FormCol><Input label="Presentación" value={form.presentacion} onChange={e => set('presentacion', e.target.value)} placeholder="Cápsulas, Inyectable…" /></FormCol>
            </FormRow>
            <FormRow>
              <FormCol>
                <Input label="Stock" value={form.stock} error={errors.stock}
              ...
              <FormCol>
                <Input label="Precio unitario (COP) *" value={form.precio_unit} error={errors.precio_unit}
                  allowPattern={PATTERNS.soloNumeros}
                  onChange={e => set('precio_unit', e.target.value)}
                  placeholder="15000"
                />
              </FormCol>
            </FormRow>
...
export function FacturasPage() {
  const { data, loading } = useFetch('/facturas')
  return (
    <div className="fade-in">
    ...
    <div className="fade-in">
      ...
      {showModal && (
        <Modal title={editing ? `Editar: ${editing.username}` : 'Nuevo Usuario'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {!editing && <>
              <Input label="Username *" value={form.username} error={errors.username} onChange={e => set('username', e.target.value)} placeholder="Ej: dr_carlos" />
              <Input label="Email *" type="email" value={form.email} error={errors.email} onChange={e => set('email', e.target.value)} placeholder="carlos@vetcore.com" />
              <Input label="Contraseña * (mín. 8 caracteres)" type="password" value={form.password} error={errors.password} onChange={e => set('password', e.target.value)} />
            </>}
    ...
    <div className="fade-in" style={{ display: 'flex', gap: 20, height: 'calc(100vh - 80px)' }}>
      ...
      <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <Card style={{ padding: '12px 14px' }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-muted)', textTransform: 'uppercase', letterSpacing: '.4px', marginBottom: 8 }}>20 Consultas predefinidas</p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
        <Card style={{ padding: '14px 16px' }}>
          ...
          <textarea
            ...
            placeholder="Escribe tu SELECT aquí… (Ctrl+Enter para ejecutar)"
        ...
        {results === null && !error && (
          <Card style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center', color: 'var(--ink-faint)' }}>
              ...
              <p style={{ fontSize: 13 }}>Selecciona una consulta predefinida o escribe tu propio SELECT</p>
              <p style={{ fontSize: 11, marginTop: 6, color: 'var(--ink-faint)' }}>Ctrl+Enter para ejecutar</p>
            </div>
          </Card>
        )}
      </div>
