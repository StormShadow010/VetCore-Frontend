...
import type { Cita, Mascota, Veterinario, EstadoCita } from '../types'
...
export default function CitasPage() {
  ...
  const [form, setForm] = useState({ id_mascota: '', id_veterinario: '', fecha_hora: '', motivo: '', observaciones: '' })

  const { data: citas, loading, refetch } = useFetch<Cita[]>(`/citas${estado ? `?estado=${estado}` : ''}`, [estado])
  const { data: mascotas } = useFetch<Mascota[]>('/mascotas')
  const { data: veterinarios } = useFetch<Veterinario[]>('/veterinarios')

  const set = useCallback((k: string, v: string) => setForm(f => ({ ...f, [k]: v })), [])

  const filtered = (citas ?? []).filter(c =>
    !search || c.mascota_nombre.toLowerCase().includes(search.toLowerCase()) ||
    c.propietario_nombre.toLowerCase().includes(search.toLowerCase())
  )

  const cambiarEstado = async (id: number, nuevoEstado: EstadoCita) => {
    try { await api.patch(`/citas/${id}/estado`, { estado: nuevoEstado }); refetch() }
    catch (e) { alert((e as Error).message) }
  }

  const handleCreate = async () => {
    if (!form.id_mascota || !form.id_veterinario || !form.fecha_hora) { setFormError('Mascota, veterinario y fecha son obligatorios'); return }
    setSaving(true); setFormError('')
    try {
      await api.post('/citas', { ...form, id_mascota: Number(form.id_mascota), id_veterinario: Number(form.id_veterinario) })
      setShowModal(false)
      setForm({ id_mascota: '', id_veterinario: '', fecha_hora: '', motivo: '', observaciones: '' })
      refetch()
      ...
      <PageHeader title="Citas Médicas" action={
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <select value={estado} onChange={e => setEstado(e.target.value)} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '8px 11px', fontSize: 13, outline: 'none', background: 'var(--surface)', color: 'var(--ink)' }}>
            ...
            {(['PENDIENTE', 'ATENDIDA', 'CANCELADA', 'NO_ASISTIO'] as EstadoCita[]).map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          {can('USUARIO') && <Btn onClick={() => { setFormError(''); setShowModal(true) }}>+ Nueva cita</Btn>}
        </div>
      } />
      <Card>
        {loading ? <Spinner /> : (
          <Table
            ...
            rows={filtered.map(c => [
              ...
              <Badge label={c.estado} variant={ESTADO_BADGE[c.estado]} />,
              ...(can('USUARIO') ? [
                c.estado === 'PENDIENTE'
      ...
      {showModal && (
        <Modal title="Nueva Cita" onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
            <Select label="Mascota *" value={form.id_mascota} onChange={e => set('id_mascota', e.target.value)}>
              <option value="">Seleccionar…</option>
              {(mascotas ?? []).map(m => <option key={m.id_mascota} value={m.id_mascota}>{m.nombre} — {m.propietario_nombre}</option>)}
            </Select>
            <Select label="Veterinario *" value={form.id_veterinario} onChange={e => set('id_veterinario', e.target.value)}>
              <option value="">Seleccionar…</option>
              {(veterinarios ?? []).map(v => <option key={v.id_veterinario} value={v.id_veterinario}>{v.nombres} {v.apellidos} — {v.especialidad_nombre}</option>)}
            </Select>
            <Input label="Fecha y hora *" type="datetime-local" value={form.fecha_hora} onChange={e => set('fecha_hora', e.target.value)} />
            <Input label="Motivo" value={form.motivo} onChange={e => set('motivo', e.target.value)} placeholder="Vacunación, revisión general…" />
