...
export default function Hero() {
  return (
    <section style={{ minHeight: "calc(100vh - 76px)", background: "var(--background)", display: "flex", alignItems: "center", padding: "3rem 4rem", transition: "background .25s" }}>
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center w-full">
        ...
        <div className="flex flex-col gap-6 max-w-xl">
          ...
          <div>
            <span style={{ display: "inline-block", padding: "6px 14px", borderRadius: 99, background: "var(--accent-light)", color: "var(--accent-dark)", fontSize: 12, fontWeight: 600, letterSpacing: ".3px" }}>
          ...
          <div className="flex items-center gap-4 mt-2">
            <button className="px-8 py-3.5 rounded-full bg-emerald-700 text-white font-semibold hover:bg-emerald-800 transition-all shadow-md shrink-0">
              Ver servicios
            </button>
            <button className="px-8 py-3.5 rounded-full border border-[var(--border)] text-[var(--ink)] font-semibold bg-[var(--accent-light)]/50 hover:bg-[var(--accent-light)] transition-all shrink-0">
              Agendar cita
            </button>
          </div>
